...
class ObsConstruct:
    ...
    def obs_construct(self, obs_raw_dict):
        ...
        for i in range(self.fighter_num):
            ...
            self.obs_dict['strike_list'] = obs_raw_dict['joint_obs_dict']['strike_list']
            ...
            for j in range(len(self.obs_dict[i]['r_visible_list'])):
                ...
                enemy_dict['distance'] = ut.distance(
                    myself_pos_x, myself_pos_y, enemy_pos_x, enemy_pos_y)
                enemy_dict['angle'] = ut.angle(
                    myself_pos_x, myself_pos_y, enemy_pos_x, enemy_pos_y)
            ...
            if len(detected_enemy_list) > 0:
                detected_enemy_list = sorted(
                    detected_enemy_list, key=lambda k: k['distance'])
                self.obs_dict[i]['target_list'] = detected_enemy_list
        return self.obs_dict

    @staticmethod
    def distance_key(ele):
        return ele['distance']

    def obs_construct_old(self, obs_raw_dict):
        obs_dict = {}
        detector_obs_list = []
        fighter_obs_list = []
    ...
    def __get_img_obs(self, detector_data_obs_list, fighter_data_obs_list, joint_data_obs_dict):
        img_obs_size_x = int(self.battlefield_size_y /
                             self.img_obs_reduce_ratio)
        img_obs_size_y = int(self.battlefield_size_x /
                             self.img_obs_reduce_ratio)
        # 个体img：所有己方单位位置
        detector_img = np.full(
            (self.detector_num, img_obs_size_x, img_obs_size_y, 3), 0, dtype=np.int32)
        fighter_img = np.full(
            (self.fighter_num, img_obs_size_x, img_obs_size_y, 3), 0, dtype=np.int32)
        # 全局img：所有可见敌方单元位置和类型
        joint_img = np.full(
            (1, img_obs_size_x, img_obs_size_y, 2), 0, dtype=np.int32)

        # set all self unit pos, detector: 1, fighter: 2, self: 255
        tmp_pos_obs = np.full(
            (img_obs_size_x, img_obs_size_y), 0, dtype=np.int32)
        for x in range(self.detector_num):
            if not detector_data_obs_list[x]['alive']:
                continue
