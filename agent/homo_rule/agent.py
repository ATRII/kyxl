...
class Agent(BaseAgent):
    ...
    def set_map_info(self, size_x, size_y, detector_num, fighter_num):
        ...
        if self.detector_num != 0:
            print(
                'Error: agents using airc_homo_rule can only be used in a homogeneous environment!')
            exit(1)
        ...
        self.to_explore_set = set(
            range((size_x // SQUARE_SIZE) * (size_y // SQUARE_SIZE)))
    ...
    def get_action(self, obs_dict, step_cnt):
        ...
        if step_cnt == 1:
            for i in range(self.fighter_num):
                self.explore_destination_set[i] = (
                    self.size_x - obs_dict[i]['pos_x'], obs_dict[i]['pos_y'])
        detector_action = []
        fighter_action = []
            ...
            if obs_dict[index]['alive']:
                try:
                    if self.rule_set == 'random':
                        tentative_action = self.choose_action_random(
                            index, step_cnt)
                    elif self.rule_set == 'random_attack':
                        tentative_action = self.choose_action_random_attack(
                            obs_dict, index, step_cnt)
                    elif self.rule_set == 'explore':
                        tentative_action = self.choose_action_explore(
                            obs_dict, index)
                    elif self.rule_set == 'attack':
                        tentative_action = self.choose_action_attack(
                            obs_dict, index)
                    elif self.rule_set == 'still':
                        tentative_action = self.choose_action_still(step_cnt)
                except AttributeError:
                    print(
                        "agent is missing a rule_set attribute. Now flying towards right as default.")
                # action formation
    ...
    def choose_action_explore(self, obs_dict, index):
        ...
        if explored_index >= (self.size_x / SQUARE_SIZE) * (self.size_y / SQUARE_SIZE):
            ...
            explored_index = (self.size_x / SQUARE_SIZE) * \
                (self.size_y / SQUARE_SIZE) - 1
        if explored_index in self.to_explore_set:
            self.to_explore_set.remove(explored_index)
            if len(self.to_explore_set) == 0:
                self.to_explore_set = set(
                    range(round((self.size_x / SQUARE_SIZE) * (self.size_y / SQUARE_SIZE))))
